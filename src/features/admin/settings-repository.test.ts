...
describe("settings repository", () => {
  it("returns locked defaults and 30-day retention", () => {
    const db = database();
    expect(getAccessSettings(db, new Date("2026-09-22T00:00:00Z"))).toEqual({
      requireSignIn: true,
      expiresAt: null,
      reason: null,
    });
  ...
  it("rejects retention values outside the 1-365 day range", () => {
    const db = database();
    expect(() => setSetting(db, "recycle_retention_days", 0, null)).toThrow(
      "INVALID_SETTING"
    );
    expect(() => setSetting(db, "recycle_retention_days", 366, null)).toThrow(
      "INVALID_SETTING"
    );
    db.close();
  });
  ...
  it("reports anonymous access only while the persisted window is active", () => {
    ...
    expect(
      getAccessSettings(db, new Date("2026-09-22T00:30:00Z"))
    ).toMatchObject({
      requireSignIn: false,
      expiresAt: "2026-09-22T01:00:00.000Z",
    });
    expect(
      getAccessSettings(db, new Date("2026-09-22T02:00:00Z"))
    ).toMatchObject({ requireSignIn: true, expiresAt: null });
    db.close();
  });
});

...
let root: string;
let admin: { id: string; username: string; displayName: string; role: "user" | "admin"; mustChangePassword: boolean };
...
beforeEach(async () => {
  ...
  migrate(db);
  admin = await createAdministrator(db, { username: "admin", displayName: "Admin", password: "correct-password" });
});
...
describe("admin user service", () => {
  it("creates users and prevents case-insensitive duplicates", async () => {
    ...
    expect(user).toMatchObject({ username: "alex", role: "user", mustChangePassword: false });
    await expect(createUser(db, actor(), { username: "ALEX", displayName: "Alex 2", password: "correct-password" })).rejects.toThrow("USERNAME_EXISTS");
  });
  it("protects the last active admin and revokes disabled sessions", async () => {
    const session = createSession(db, admin.id, new Date());
    await expect(setUserDisabled(db, actor(), admin.id, true)).rejects.toThrow("LAST_ADMIN");
    const second = await createUser(db, actor(), { username: "second", displayName: "Second", password: "correct-password", role: "admin" });

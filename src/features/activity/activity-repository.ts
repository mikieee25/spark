...
const SENSITIVE_KEY =
  /password|secret|token|credential|authorization|cookie|content/i;
...
export function recordActivity(
  ...
  const event: ActivityEvent = {
    ...
    metadata: (redactMetadata(input.metadata ?? {}) ?? {}) as Record<
      string,
      unknown
    >,
  };
...
export function listActivity(
  ...
  const rows = database
    ...
    .all(before, before, limit) as Array<Record<string, unknown>>;

...
describe("activity query and export", () => {
  it("exports a fixed escaped safe column set", () => {
    ...
    expect(csv).toContain(
      "occurred_at,actor_type,actor_username,action,paths,outcome,error_code"
    );
    expect(csv).toContain('"a,b"');
    expect(csv).not.toContain("hidden");
  });
  it("returns bounded deterministic activity results", () =>
    expect(queryActivity({} as never, { limit: 0 })).toMatchObject({
      items: [],
      nextCursor: null,
    }));
});

...
): Promise<{ user: SessionUser | null; actorType: "user" | "anonymous" }> {
  const user = await getCurrentUser();
  if (user) {
    if (user.mustChangePassword) throw new Error("PASSWORD_CHANGE_REQUIRED");
    if (capability === "admin" && user.role !== "admin")
      throw new Error("ADMIN_REQUIRED");
    return { user, actorType: "user" };
  }
  let state: AccessState;
  try {
    state = getAccessState(getDatabase());
  } catch {
    throw new Error("UNAUTHENTICATED");
  }
  if (
    state.anonymous &&
    ["browse", "search", "preview", "download"].includes(capability)
  )
    return { user: null, actorType: "anonymous" };
  throw new Error(
    state.requireSignIn ? "UNAUTHENTICATED" : "ANONYMOUS_ACCESS_EXPIRED"
  );
}

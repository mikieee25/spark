...
describe("recycle maintenance", () => {
  it("expires and purges expired private content with an audit event", async () => {
    const storage = {
      removePrivate: async (key: string) =>
        fs.rm(path.join(directory, "recycle", key)),
    };
    await runRecycleMaintenance(
      database,
      storage,
    ...
    expect(
      database.prepare("SELECT purged_at FROM recycle_entries").get()

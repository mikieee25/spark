...
export async function createFolder(
  path: string,
  signal?: AbortSignal
): Promise<{ item: FileEntry }> {
  return jsonRequest("/api/files/folders", {
    method: "POST",
...
export async function recycleSelection(
  paths: readonly string[]
): Promise<BatchRecycleResult> {
  return (await (
    await batchRequest("recycle", paths)
  ).json()) as BatchRecycleResult;
}

export async function restoreRecycleItem(id: string): Promise<unknown> {
  return jsonRequest(`/api/recycle/${encodeURIComponent(id)}/restore`, {
    method: "POST",
    body: "{}",
  });
}
...
type FolderUploadProgress = Readonly<{
  completed: number;
  total: number;
  logicalPath: string;
}>;
export type FolderUploadItem =
  File | Readonly<{ file: File; relativePath: string }>;
...
export async function uploadFolder(input: {
  ...
  onConflict?: (
    conflict: FolderUploadConflict
  ) => Promise<ConflictPolicy | "cancel">;
  signal?: AbortSignal;
}): Promise<{ uploaded: number; skipped: number }> {
  const files = input.files.filter(
    (source) => ("file" in source ? source.file : source).size >= 0
  );
  ...
  const items = files.map((source) => {
    ...
    for (let index = 1; index <= segments.length; index += 1)
      directories.add(joinPath(input.directory, ...segments.slice(0, index)));
    return {
      file,
      logicalPath: joinPath(input.directory, ...segments, name),
      directory: joinPath(input.directory, ...segments),
    };
  });
  for (const directory of [...directories].sort(
    ...
    try {
      await createFolder(directory, input.signal);
    } catch (error) {
      if (error instanceof FileApiError && error.code === "UPLOAD_CANCELLED")
        throw error;
    ...
    while (true) {
      try {
        ...
        if (input.signal?.aborted)
          throw new FileApiError("UPLOAD_CANCELLED", 409);
        if (
          ...
          throw error;
        const choice = await input.onConflict({
          file: item.file,
          logicalPath: item.logicalPath,
        });
        if (choice === "cancel")

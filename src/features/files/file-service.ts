...
async function withConflict(
  ...
  for (let index = 1; index < 10_000; index += 1) {
    ...
    const candidate =
      directory === "." ? candidateName : `${directory}/${candidateName}`;
...
export function listFileVersions(
  ...
  const rows = database
    .prepare(
      `SELECT id, original_path, storage_key, size_bytes, created_at
    FROM file_versions WHERE original_path = ? ORDER BY created_at DESC, id DESC`
    )
    .all(originalPath) as Array<Record<string, unknown>>;
  return rows.map((row) => ({
    id: row.id as string,
...
export function createFileService({
  ...
  async function run<T>(
    ...
    type: string,
    paths: string[],
    callback: (operation: FileOperation) => Promise<T>
  ): Promise<T> {
    const operation = beginOperation(database, {
  ...
  return {
    ...
    async uploadFile(actor: FileActor, input: UploadInput) {
      ...
      return run(actor, "file_upload", [initial], async (operation) => {
        ...
        const destination = await withConflict(
          storage,
          initial,
          input.conflict
        );
        if (!destination)
          return { operationId: operation.id, skipped: true } as const;
        if (destination === initial && input.conflict === "replace")
          await captureVersion(operation, actor, destination);
    ...
    async moveFile(
      actor: FileActor,
      input: { source: string; destination: string; conflict?: ConflictPolicy }
    ) {
      ...
      return run(
        actor,
        "file_move",
        [source, requestedDestination],
        async (operation) => {
          const sourceItem = await storage.stat(source);
          if (
            source === requestedDestination ||
            (sourceItem.kind === "folder" &&
              requestedDestination.startsWith(`${source}/`))
          ) {
    ...
    async restoreFromRecycle(
      actor: FileActor,
      input: { id: string; conflict?: ConflictPolicy }
    ) {
    ...
    async list(
      ...
      try {
        return await storage.list(normalizeLogicalPath(pathName), options);
      } finally {
        release();
      }
    },

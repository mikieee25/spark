      ...
      <Card className="flex min-h-0 min-w-0 flex-col overflow-hidden shadow-none">
        <CardHeader className="shrink-0 gap-4 border-b bg-muted/20 px-4 py-4 sm:px-6">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <CardTitle className="text-base">{currentPath || "My workspace"}</CardTitle>
    ...
    <Sheet open={Boolean(selected)} onOpenChange={(open) => { if (!open) setSelected(null); }}>
      {selected && <SheetContent aria-label="Selected item details" side="right" showCloseButton={false} className="w-full gap-0 overflow-y-auto p-0 sm:w-3/4 sm:!max-w-xl max-sm:inset-x-0 max-sm:inset-y-auto max-sm:top-auto max-sm:bottom-0 max-sm:h-[85dvh] max-sm:w-full max-sm:max-w-none max-sm:rounded-t-2xl max-sm:border-l-0 max-sm:border-t">
        <SheetHeader className="shrink-0 flex-row items-start justify-between gap-4 border-b px-4 py-4">
          <div className="min-w-0"><SheetTitle className="sr-only">Selected item details</SheetTitle><p className="truncate font-heading text-base font-medium">{selected.name}</p><SheetDescription>{selected.kind === "folder" ? "Folder" : "File"}{selected.sizeBytes > 0 ? ` · ${formatSize(selected.sizeBytes)}` : ""}</SheetDescription></div>
          <Button variant="ghost" size="icon-sm" aria-label="Close details" onClick={() => setSelected(null)}><X /></Button>
        </SheetHeader>
        <div className="flex min-h-0 flex-col gap-4 overflow-y-auto p-4">{selected.kind === "file" ? <PreviewPanel item={selected as PreviewItem} onPreviewed={refreshRecent} /> : <section aria-label={`Preview of ${selected.name}`} className="flex min-h-56 items-center justify-center rounded-lg border border-dashed bg-muted/20 p-6"><Folder className="size-16 text-primary" aria-hidden="true" /></section>}<Button variant="outline" onClick={() => void toggleFavorite()}><Star data-icon="inline-start" fill={selectedIsFavorite ? "currentColor" : "none"} />{selectedIsFavorite ? "Remove from favorites" : "Add to favorites"}</Button><a className={buttonVariants()} href={downloadUrl(selected.logicalPath)}><ArrowDownToLine data-icon="inline-start" />{selected.kind === "file" ? "Download" : "Download folder"}</a><Button variant="outline" onClick={() => { setRenameName(selected.name); setRenameOpen(true); }}>Rename</Button><Button variant="destructive" onClick={() => setDeleteOpen(true)}><Trash2 data-icon="inline-start" />Move to Recycle bin</Button></div>
      </SheetContent>}
    </Sheet>

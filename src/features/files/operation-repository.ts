...
export type OperationState =
  "pending" | "completed" | "failed" | "recovery_required";
...
export function completeOperation(
  ...
  database
    .prepare(
      "UPDATE file_operations SET state = 'completed', completed_at = ?, error_code = NULL WHERE id = ? AND state = 'pending'"
    )
...
export function failOperation(
  ...
  database
    .prepare(
      "UPDATE file_operations SET state = 'failed', completed_at = ?, error_code = ? WHERE id = ? AND state = 'pending'"
    )
...
export function listIncompleteOperations(
  ...
  return (
    database
      .prepare(
        "SELECT * FROM file_operations WHERE state = 'pending' ORDER BY created_at, id"
      )
      .all() as OperationRow[]
  ).map(toOperation);
}
...
export async function withPathLocks<T>(
  ...
  try {
    database.transaction(() => {
      const statement = database.prepare(
        "INSERT INTO operation_path_locks(path, operation_id, created_at) VALUES (?, ?, ?)"
      );
  ...
  try {
    ...
    database
      .prepare("DELETE FROM operation_path_locks WHERE operation_id = ?")
      .run(operation.id);
  }
}

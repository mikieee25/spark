...
describe("operation repository", () => {
  it("persists pending, completed, and failed mutations", () => {
    const completed = beginOperation(database, {
      actorUserId: "user-1",
      type: "folder_create",
      paths: ["Reports"],
    });
    completeOperation(database, completed.id);
    const failed = beginOperation(database, {
      actorUserId: "user-1",
      type: "upload",
      paths: ["Reports/a.txt"],
    });
    ...
    expect(
      database
        .prepare(
          "SELECT state, error_code FROM file_operations ORDER BY created_at"
        )
        .all()
    ).toEqual([
      { state: "completed", error_code: null },
      { state: "failed", error_code: "CONFLICT" },
    ]);
  });
  ...
  it("locks paths deterministically for a mutation", async () => {
    ...
    await withPathLocks(database, operation, ["b", "a"], async () => {
      expect(
        database
          .prepare("SELECT path FROM operation_path_locks ORDER BY path")
          .all()
      ).toEqual([{ path: "a" }, { path: "b" }]);
    });
    expect(
      database.prepare("SELECT count(*) count FROM operation_path_locks").get()
    ).toEqual({ count: 0 });
  });
});

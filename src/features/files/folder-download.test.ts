  ...
  return {
    list: async (logicalPath: string) => {
      const entries = logicalPath === "Reports"
        ? [{ name: "2026", logicalPath: "Reports/2026", kind: "folder" as const, sizeBytes: 0, modifiedAt: "2026-09-22T00:00:00.000Z" }, { name: "readme.md", logicalPath: "Reports/readme.md", kind: "file" as const, sizeBytes: 7, modifiedAt: "2026-09-22T00:00:00.000Z" }]
        : [{ name: "brief.txt", logicalPath: "Reports/2026/brief.txt", kind: "file" as const, sizeBytes: 9, modifiedAt: "2026-09-22T00:00:00.000Z" }];
      return entries;
    },
    stat: async (logicalPath: string) => logicalPath === "Reports"
      ? { name: "Reports", logicalPath, kind: "folder", sizeBytes: 0, modifiedAt: "2026-09-22T00:00:00.000Z" }
      : (() => { const bytes = files.get(logicalPath); if (!bytes) throw new Error("NOT_FOUND"); return { name: logicalPath.split("/").at(-1)!, logicalPath, kind: "file" as const, sizeBytes: bytes.byteLength, modifiedAt: "2026-09-22T00:00:00.000Z" }; })(),
    readFile: async (logicalPath: string) => files.get(logicalPath) ?? (() => { throw new Error("NOT_FOUND"); })(),
...
describe("folder download", () => {
  ...
  it("rejects a file as a folder archive source", async () => {
    ...
    await expect(createFolderArchive(storage, "Reports/readme.md")).rejects.toThrow("NOT_A_DIRECTORY");
  });

  it("propagates a missing folder and rejects unsafe archive entries", async () => {
    const missing = storageFixture();
    await expect(createFolderArchive({ ...missing, stat: async () => { throw new Error("NOT_FOUND"); } } as StorageAdapter, "Missing")).rejects.toThrow("NOT_FOUND");
    const unsafe = { ...storageFixture(), list: async () => [{ name: "evil.txt", logicalPath: "Reports/../evil.txt", kind: "file" as const, sizeBytes: 1, modifiedAt: "2026-09-22T00:00:00.000Z" }] } as unknown as StorageAdapter;
    await expect(createFolderArchive(unsafe, "Reports")).rejects.toThrow("INVALID_ARCHIVE_PATH");
  });

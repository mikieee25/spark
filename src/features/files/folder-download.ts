...
function relativeArchivePath(root: string, logicalPath: string): string {
  if (root && logicalPath !== root && !logicalPath.startsWith(`${root}/`))
    throw new Error("INVALID_ARCHIVE_PATH");
  const relative = root
    ? logicalPath === root
      ? ""
      : logicalPath.slice(root.length + 1)
    : logicalPath;
  if (
    !relative ||
    relative.includes("\\") ||
...
export async function createFolderArchive(
  ...
  async function collect(directory: string): Promise<void> {
    for (const entry of await storage.list(directory)) {
      ...
      if (Object.keys(files).length >= MAX_FOLDER_ARCHIVE_FILES)
        throw new Error("ARCHIVE_TOO_LARGE");
      totalBytes += entry.sizeBytes;
      if (totalBytes > MAX_FOLDER_ARCHIVE_BYTES)
        throw new Error("ARCHIVE_TOO_LARGE");
      files[relativeArchivePath(root, entry.logicalPath)] =
        await storage.readFile(entry.logicalPath);
    }
  }
...
export async function createSelectionArchive(
  storage: StorageAdapter,
  ...
  const overlaps =
    selected.size !== paths.length ||
    paths.some((logicalPath) => {
  ...
  async function addFolder(root: string, archiveRoot: string): Promise<void> {
    for (const entry of await storage.list(root)) {
      ...
      if (entry.kind === "folder")
        await addFolder(entry.logicalPath, archivePath);
  ...
  return {
    body: Buffer.from(zipSync(files, { level: 0 })),

...
async function insertUser(disabled = false): Promise<void> {
  ...
  database.prepare(`INSERT INTO users
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(
      randomUUID(), "Alice", "Alice DOE", await hashPassword("correct-password"),
      "user", disabled ? now : null, now, now,
    );
}
...
describe("authenticate", () => {
  it("authenticates usernames case-insensitively without credential fields", async () => {
    ...
    const result = await authenticate(
      database, "alice", "correct-password", new Date("2026-09-21T01:00:00Z"),
    );
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.user).toEqual(expect.objectContaining({ username: "Alice" }));
  ...
  it("rejects unknown, incorrect, and disabled accounts", async () => {
    await insertUser();
    await expect(authenticate(database, "missing", "incorrect-pass", new Date()))

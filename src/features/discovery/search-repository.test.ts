...
function createDatabase() {
  ...
  const add = (path: string, name: string, kind: "file" | "folder", text = "") => {
    database.prepare(`INSERT INTO file_index_entries
      ...
      .run(path, path.includes("/") ? path.slice(0, path.lastIndexOf("/")) : "", name, kind, now, text ? 1 : 0, now);
    database.prepare("INSERT INTO file_index_fts(logical_path, name, text_content) VALUES (?, ?, ?)").run(path, name, text);
  };
...
describe("search repository", () => {
  ...
  it("returns bounded deterministic pages with an opaque cursor", () => {
    ...
    expect(searchFiles(database, { query: "alpha", limit: 1, cursor: first.nextCursor }).items.map((item) => item.logicalPath))
      .toEqual(["photos/alpha.png"]);
  });

  it("rejects malformed cursors and unsafe FTS operators", () => {
    const { database } = createDatabase();
    expect(() => searchFiles(database, { query: 'alpha OR *', cursor: "not-a-cursor" })).toThrow("INVALID_CURSOR");
    expect(searchFiles(database, { query: 'alpha OR *' }).items.map((item) => item.logicalPath)).toEqual([
      "docs/alpha.txt", "photos/alpha.png",
    ]);
  });

  it.each(["../secret", "a//b", "a\\b", "/absolute", "CON/file.txt"]) ("rejects unsafe path %s", (pathPrefix) => {
    const { database } = createDatabase();
    expect(() => searchFiles(database, { query: "alpha", pathPrefix })).toThrow("INVALID_PATH");
  });

  it("rejects unsafe decoded cursor paths", () => {
    const { database } = createDatabase();
    const cursor = Buffer.from(JSON.stringify({ logicalPath: "../secret" })).toString("base64url");
    expect(() => searchFiles(database, { query: "alpha", cursor })).toThrow("INVALID_CURSOR");
  });
});

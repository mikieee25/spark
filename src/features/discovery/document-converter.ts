...
export function createDocumentConverter(url: URL): DocumentConverter {
  return {
    async convert(name, bytes) {
      ...
      try {
        response = await fetch(url, { method: "POST", body: form, signal: AbortSignal.timeout(CONVERTER_TIMEOUT_MS) });
      } catch {
      ...
      if (length > MAX_CONVERTED_PREVIEW_BYTES) throw new Error("PREVIEW_TOO_LARGE");
      const body = Buffer.from(await response.arrayBuffer());
      if (body.byteLength > MAX_CONVERTED_PREVIEW_BYTES || body.byteLength < 5 || body.subarray(0, 5).toString() !== "%PDF-") throw new Error("PREVIEW_CONVERTER_INVALID_RESPONSE");
      return body;
    },
  };
}

...
export function PreviewPanel({
  item,
  onPreviewed,
}: Readonly<{ item: PreviewItem; onPreviewed?: () => void }>) {
  ...
  const mediaKind =
    mime.startsWith("image/") || IMAGE_EXTENSIONS.has(ext)
      ? "image"
      : mime.startsWith("audio/") || AUDIO_EXTENSIONS.has(ext)
        ? "audio"
        : mime.startsWith("video/") || VIDEO_EXTENSIONS.has(ext)
          ? "video"
          : mime === "application/pdf" || ext === "pdf"
            ? "pdf"
            : CONVERTED_EXTENSIONS.has(ext)
              ? "converted"
              : null;
  const current =
    result.path === item.logicalPath ? result : { path: item.logicalPath };
  ...
  useEffect(() => {
    ...
    fetch(previewUrl(item.logicalPath), { signal: controller.signal })
      .then(async (response) => {
        ...
        if (
          !["text", "document", "spreadsheet", "archive"].includes(result.kind)
        )
      ...
      .catch((error: unknown) => {
        if (!(error instanceof DOMException && error.name === "AbortError"))
          setResult({ path: item.logicalPath, failed: true });
      });
  ...
  const fallback = (
    <div className="flex min-h-56 flex-col items-center justify-center gap-3 rounded-lg border border-dashed bg-muted/20 p-6 text-center">
      ...
      <p className="max-w-sm text-sm text-muted-foreground">
        This format cannot be shown safely here. Download it to open with a
        compatible app.
      </p>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      {!loading && !failed && mediaKind === "image" && (
        <Image
          unoptimized
          width={1000}
          height={700}
          src={`/api/files/thumbnail?path=${encodeURIComponent(item.logicalPath)}`}
          alt={`Preview of ${item.name}`}
          className="max-h-[28rem] w-full rounded-lg border bg-muted/20 object-contain"
          onLoad={onPreviewed}
          onError={() => setResult({ path: item.logicalPath, failed: true })}
        />
      )}
      {!loading && !failed && mediaKind === "pdf" && (
        <PdfPreview
          src={previewUrl(item.logicalPath)}
          title={`Preview of ${item.name}`}
          onReady={onPreviewed}
          onError={() => setResult({ path: item.logicalPath, failed: true })}
        />
      )}
      {!loading && !failed && mediaKind === "converted" && (
        <iframe
          src={previewUrl(item.logicalPath, true)}
          title={`Preview of ${item.name}`}
      ...
      {!loading && !failed && mediaKind === "audio" && (
        <audio
          controls
          src={previewUrl(item.logicalPath)}
          className="w-full"
          onLoadedData={onPreviewed}
        >
          Audio preview is unavailable.
        </audio>
      )}
      {!loading && !failed && mediaKind === "video" && (
        <video
          controls
          src={previewUrl(item.logicalPath)}
          className="max-h-[28rem] w-full rounded-lg border bg-muted/20"
          onLoadedData={onPreviewed}
        >
          Video preview is unavailable.
        </video>
      )}
      {!loading && !failed && data?.kind === "text" && (
        <div className="overflow-hidden rounded-lg border bg-muted/20">
          <div className="flex items-center gap-2 border-b px-3 py-2 text-xs text-muted-foreground">
            <FileText className="size-4" aria-hidden="true" />
            {data.truncated
              ? "Preview truncated to the safe limit"
              : "Text preview"}
          </div>
      ...
      {!loading && !failed && data?.kind === "document" && (
      ...
      {!loading && !failed && data?.kind === "spreadsheet" && (
                            ...
                            {row.map((cell, cellIndex) =>
                              rowIndex === 0 ? (
                                <th
                                  ...
                                  className="bg-muted/30 px-3 py-2 text-left font-medium"
                                >
                                  {cell}
                                </th>
                              ) : (
      ...
      {!loading && !failed && data?.kind === "archive" && (
        <div className="overflow-hidden rounded-lg border">
          <div className="flex items-center gap-2 border-b bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
            <Archive className="size-4" aria-hidden="true" />
            {data.entries.length} archive entries
          </div>
          <ScrollArea className="h-72">
      ...
      {!loading &&
        ...
        fallback}
      <a
        href={downloadUrl(item.logicalPath)}

...
import type {
  StorageAdapter,
  StorageEntry,
} from "@/features/files/storage-adapter";
import { waitForFolderReadsToFinish } from "./folder-read-priority";

type IndexerOptions = Readonly<{
...
async function nextEntries(
  ...
  cursor: string | null,
  limit: number
): Promise<{ entries: StorageEntry[]; complete: boolean }> {
  const entries: StorageEntry[] = [];
  async function visit(parentPath: string): Promise<boolean> {
    await waitForFolderReadsToFinish();
    const children = (await storage.list(parentPath, { cache: false })).sort(
    ...
    for (const entry of children) {
      ...
      const inCursorSubtree =
        cursor &&
        entry.kind === "folder" &&
        cursor.startsWith(`${entry.logicalPath}/`);
      if (!beforeCursor && !atCursor) entries.push(entry);
      if (
        entry.kind === "folder" &&
        (inCursorSubtree || !cursor || atCursor || !beforeCursor)
      ) {
...
export async function runIndexMaintenance({
  ...
  storage,
  maxEntries = 100,
}: IndexerOptions): Promise<IndexRun> {
  const limit = Math.max(1, Math.floor(maxEntries));
  const row = database
    .prepare(
      "SELECT generation, cursor, status FROM file_index_state WHERE id = 1"
    )
    .get() as { generation: number; cursor: string | null; status: string };
  const generation =
    row.status === "idle" ? row.generation + 1 : row.generation;
  const cursor = row.status === "idle" ? null : row.cursor;
  let checkpoint = cursor;
  updateIndexState(database, {
    generation,
    cursor,
    status: "running",
    error: null,
  ...
  try {
    await waitForFolderReadsToFinish();
    const { entries: pending, complete } = await nextEntries(
      storage,
      cursor,
      limit
    );
    for (const entry of pending) {
      ...
      const parentPath = entry.logicalPath.includes("/")
        ? entry.logicalPath.slice(0, entry.logicalPath.lastIndexOf("/"))
        : "";
      const textIndexed =
        entry.kind === "file" &&
        classification.text &&
        entry.sizeBytes <= MAX_INDEXED_TEXT_BYTES;
      ...
      if (textIndexed && !contentUnchanged) {
        await waitForFolderReadsToFinish();
        const text = (await storage.readFile(entry.logicalPath))
          .subarray(0, MAX_INDEXED_TEXT_BYTES)
          .toString("utf8");
    ...
    if (complete) {
      database.transaction(() => {
        database
          .prepare("DELETE FROM file_index_entries WHERE generation <> ?")
          .run(generation);
        database
          .prepare(
            "DELETE FROM file_index_fts WHERE logical_path NOT IN (SELECT logical_path FROM file_index_entries)"
          )
          .run();
        updateIndexState(database, {
          generation,
    ...
    updateIndexState(database, {
      generation,
      cursor: checkpoint ?? (row.status === "idle" ? null : row.cursor),
      status: "error",
      error: message,
    });

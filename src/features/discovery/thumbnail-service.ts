  ...
  for (let start = 0; start < size; start += MAX_STORAGE_RANGE_BYTES)
    chunks.push(
      await storage.readRange(
        logicalPath,
        start,
        Math.min(size - 1, start + MAX_STORAGE_RANGE_BYTES - 1)
      )
    );
...
export function createThumbnailService(
  options: Readonly<{ storage: StorageAdapter; dataDirectory: string }>
) {
  ...
  return {
    async getThumbnail(
      ...
      Readonly<{ buffer: Buffer; mimeType: "image/webp"; fromCache: boolean }>
    > {
      const item = await options.storage.stat(logicalPath);
      if (item.kind !== "file") throw new Error("NOT_A_FILE");
      if (item.sizeBytes > MAX_THUMBNAIL_SOURCE_BYTES)
        throw new Error("THUMBNAIL_TOO_LARGE");
      const classification = classifyContent(item.name);
      if (!IMAGE_MIMES.has(classification.mimeType))
        throw new Error("UNSUPPORTED_THUMBNAIL");
      const key = createHash("sha256")
        .update(`${item.logicalPath}\0${item.sizeBytes}\0${item.modifiedAt}`)
        .digest("hex");

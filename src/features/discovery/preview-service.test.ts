...
describe("preview service", () => {
  it("returns sanitized bounded DOCX HTML previews", async () => {
    const docx = zipSync({
      "[Content_Types].xml": new TextEncoder().encode(`<?xml version="1.0"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Override PartName="/word/document.xml" ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.document.main+xml"/></Types>`),
      "word/document.xml": new TextEncoder().encode(`<?xml version="1.0"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p><w:r><w:t>DOE record</w:t></w:r></w:p><w:p><w:r><w:instrText>&lt;script&gt;unsafe&lt;/script&gt;</w:instrText></w:r></w:p></w:body></w:document>`),
    });
    await fs.writeFile(path.join(root, "record.docx"), docx);
    const result = await createPreviewService(createStorageAdapter({ filesRoot: root, dataDirectory: data })).preview("record.docx");
  ...
  it("returns bounded spreadsheet sheets for XLSX", async () => {
    ...
    XLSX.utils.book_append_sheet(workbook, XLSX.utils.aoa_to_sheet([["Name", "Value"], ["DOE", 42]]), "Summary");
    await fs.writeFile(path.join(root, "summary.xlsx"), XLSX.write(workbook, { type: "buffer", bookType: "xlsx" }));
    const result = await createPreviewService(createStorageAdapter({ filesRoot: root, dataDirectory: data })).preview("summary.xlsx");
    expect(result).toMatchObject({ kind: "spreadsheet", format: "xlsx" });
    expect((result as { sheets: Array<{ name: string; rows: string[][]; truncated: boolean }> }).sheets[0]).toEqual({ name: "Summary", rows: [["Name", "Value"], ["DOE", "42"]], truncated: false });
  });
  ...
  it("converts legacy and presentation documents only when a converter is supplied", async () => {
    ...
    const converter = { convert: vi.fn().mockResolvedValue(Buffer.from("%PDF-1.7 output")) };
    const service = createPreviewService(createStorageAdapter({ filesRoot: root, dataDirectory: data }), { converter, dataDirectory: data });
    await expect(service.convert("slides.pptx")).resolves.toMatchObject({ kind: "media", mimeType: "application/pdf", body: Buffer.from("%PDF-1.7 output") });
    await expect(service.convert("slides.pptx")).resolves.toMatchObject({ kind: "media", body: Buffer.from("%PDF-1.7 output") });

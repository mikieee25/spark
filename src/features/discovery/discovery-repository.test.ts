...
describe("discovery repository", () => {
  it("stores index entries, bounded FTS content, and resumable state", () => {
    ...
    expect(getIndexState(db)).toMatchObject({
      generation: 2,
      cursor: "docs/readme.md",
      status: "running",
    });
    expect(
      db
        .prepare(
          "SELECT logical_path, name FROM file_index_fts WHERE file_index_fts MATCH 'spark'"
        )
        .get()
    ).toEqual({ logical_path: "docs/readme.md", name: "readme.md" });
    replaceIndexEntry(db, {
      logicalPath: "docs/readme.md",
      parentPath: "docs",
      name: "guide.md",
    ...
    expect(
      db
        .prepare("SELECT name FROM file_index_fts WHERE logical_path = ?")
        .get("docs/readme.md")
    ).toEqual({ name: "guide.md" });
    db.close();
  });
  ...
  it("rejects FTS content above the shared byte ceiling", () => {
    ...
    expect(() =>
      replaceIndexText(db, "docs/large.txt", "x".repeat(256 * 1024 + 1))
    ).toThrowError("INDEX_TEXT_TOO_LARGE");
    db.close();
  });
  ...
  it("isolates favorites and keeps recent items newest-first", () => {
    ...
    expect(listFavorites(db, "u1").map((item) => item.logicalPath)).toEqual([
      "a.txt",
    ]);
    expect(listRecentItems(db, "u1").map((item) => item.logicalPath)).toEqual([
      "new.txt",
      "old.txt",
    ]);
    db.close();
  ...
  it("makes favorite writes idempotent and bounds recent rows", () => {
    ...
    expect(
      listRecentItems(db, "u1").some(
        (item) => item.logicalPath === "file-0.txt"
      )
    ).toBe(false);
    db.close();
  });

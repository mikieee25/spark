...
export async function searchFiles(input: SearchFilesInput): Promise<SearchFilesResponse> {
  ...
  const response = await fetch(`/api/search?${params}`, { signal: input.signal });
  const body = await response.json().catch(() => ({})) as SearchFilesResponse & { error?: string };
  if (!response.ok) throw new FileApiError(body.error ?? "SEARCH_ERROR", response.status);
  return body;
}
...
async function discoveryRequest<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) throw new FileApiError(body.error ?? "DISCOVERY_ERROR", response.status);
  return body;
}

export async function listFavorites(signal?: AbortSignal): Promise<Favorite[]> {
  return (await discoveryRequest<{ items: Favorite[] }>("/api/discovery/favorites", { signal })).items;
}

export async function setFavorite(path: string, favorite: boolean): Promise<void> {
  await discoveryRequest("/api/discovery/favorites", { method: "PUT", body: JSON.stringify({ path, favorite }) });
}

export async function listRecent(signal?: AbortSignal): Promise<RecentItem[]> {
  return (await discoveryRequest<{ items: RecentItem[] }>("/api/discovery/recent?limit=12", { signal })).items;
}

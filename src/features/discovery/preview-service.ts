...
const ARCHIVE_FORMAT: Record<string, "zip" | "tar"> = {
  ".tar": "tar",
  ".zip": "zip",
};
const DOCUMENT_FORMAT: Record<string, "docx"> = { ".docx": "docx" };
const SPREADSHEET_FORMAT: Record<
  string,
  "csv" | "xls" | "xlsx" | "xlsb" | "xlsm" | "ods"
> = {
  ".csv": "csv",
  ".ods": "ods",
  ".xls": "xls",
...
const CONVERTIBLE_FORMAT = new Set([
  ".doc",
  ".docm",
  ".odt",
  ".odp",
...
export type PreviewResult =
  ...
  | Readonly<{
      kind: "spreadsheet";
      format: "csv" | "xls" | "xlsx" | "xlsb" | "xlsm" | "ods";
      logicalPath: string;
      mimeType: string;
...
function mediaType(
  name: string,
  classification: ReturnType<typeof classifyContent>
): string {
  return (
    MEDIA_MIME_BY_EXTENSION[path.extname(name).toLowerCase()] ??
...
function spreadsheetRows(sheet: XLSX.WorkSheet): {
  rows: string[][];
  truncated: boolean;
} {
  ...
  const truncated =
    raw.length > MAX_SPREADSHEET_PREVIEW_ROWS ||
    raw.some((row) => row.length > MAX_SPREADSHEET_PREVIEW_COLUMNS);
  return {
    rows: raw
      ...
      .map((row) =>
        row
          ...
          .map((cell) => String(cell).slice(0, MAX_SPREADSHEET_CELL_CHARS))
      ),
    truncated,
  };
}
...
export function createPreviewService(
  ...
  const convertedCache = options.dataDirectory
    ? createArtifactCache(options.dataDirectory, "converted-previews")
    : null;
  async function convert(
    logicalPath: string
  ): Promise<Extract<PreviewResult, { kind: "media" }>> {
    const item = await storage.stat(logicalPath);
    if (item.kind !== "file") throw new Error("NOT_A_FILE");
    if (!CONVERTIBLE_FORMAT.has(path.extname(item.name).toLowerCase()))
      throw new Error("UNSUPPORTED_PREVIEW");
    if (!options.converter) throw new Error("PREVIEW_CONVERTER_DISABLED");
    if (item.sizeBytes > MAX_DOCUMENT_PREVIEW_BYTES)
    ...
    const body = convertedCache
      ? await convertedCache.get(key, () =>
          options.converter!.convert(item.name, source)
        )
    ...
    return {
      ...
      body,
      start: 0,
      end: Math.max(0, body.byteLength - 1),
  ...
  return {
    ...
    async preview(
      ...
      if (archiveFormat) {
        if (item.sizeBytes > MAX_ARCHIVE_BYTES)
          throw new Error("PREVIEW_TOO_LARGE");
        ...
        return {
          ...
          logicalPath: item.logicalPath,
          mimeType:
            archiveFormat === "zip" ? "application/zip" : "application/x-tar",
          entries: listArchiveEntries(body, archiveFormat),
      ...
      if (documentFormat) {
        ...
        try {
          ...
          return {
            ...
            logicalPath: item.logicalPath,
            mimeType,
            html: sanitizeDocumentHtml(converted.value),
            sizeBytes: item.sizeBytes,
            modifiedAt: item.modifiedAt,
          };
        } catch {
      ...
      if (spreadsheetFormat) {
        ...
        try {
          const workbook = XLSX.read(
            await readBounded(storage, logicalPath, item.sizeBytes),
            { type: "buffer", cellDates: false, dense: true }
          );
          const sheets = workbook.SheetNames.slice(
            0,
            MAX_SPREADSHEET_PREVIEW_SHEETS
          ).map((name) => ({
            name,
            ...spreadsheetRows(workbook.Sheets[name]),
          }));
          return {
            kind: "spreadsheet",
      ...
      if (classification.text) {
        ...
        const body = length
          ? await storage.readRange(logicalPath, 0, length - 1)
          : Buffer.alloc(0);
        return {
          kind: "text",
          logicalPath: item.logicalPath,
          ...
          truncated: item.sizeBytes > length,
          sizeBytes: item.sizeBytes,
          modifiedAt: item.modifiedAt,
        };
      }
      ...
      if (item.sizeBytes > MAX_INLINE_MEDIA_BYTES && !range)
        throw new Error("PREVIEW_TOO_LARGE");
      if (!item.sizeBytes)
        return {
          kind: "media",
      ...
      if (range)
        return {
          ...
          logicalPath: item.logicalPath,
          mimeType,
          body: await storage.readRange(logicalPath, range.start, range.end),
          start: range.start,
          end: range.end,
      ...
      return {
        ...
        logicalPath: item.logicalPath,
        mimeType,
        stream: await storage.openReadStream(logicalPath),
        start: 0,

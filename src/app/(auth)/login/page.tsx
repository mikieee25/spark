...
export default async function LoginPage() {
  ...
  return (
    <main className="grid min-h-screen bg-[var(--canvas)] lg:grid-cols-[1.1fr_0.9fr]">
      <section className="relative hidden overflow-hidden bg-[var(--canvas-deep)] p-14 text-white lg:flex lg:flex-col lg:justify-between">
        <div className="absolute -right-32 top-20 size-96 rounded-full bg-[var(--doe-blue)] opacity-35 blur-3xl" />
        <div className="relative flex items-center gap-4">
          ...
          />
          <p className="text-sm font-black uppercase tracking-[0.25em] text-[var(--doe-yellow)]">
      ...
      <section className="flex items-center justify-center p-6 sm:p-12">
        <div className="w-full max-w-md rounded-3xl border border-[var(--line)] bg-[var(--paper)] p-7 shadow-[var(--shadow)] sm:p-10">
          <BrandLockup />
          <h2 className="mt-10 text-3xl font-black tracking-tight">Welcome back</h2>
          <p className="mt-2 text-[var(--ink-muted)]">Sign in with your local SPARK account.</p>
          <LoginForm />
        </div>
      </section>
    </main>

...
vi.mock("@/features/discovery/discovery-repository", () => ({
  listFavorites: mocks.list,
  addFavorite: mocks.add,
  removeFavorite: mocks.remove,
}));
...
describe("favorites routes", () => {
  ...
  it("lists only the current user's favorites", async () => {
    ...
    mocks.list.mockReturnValue([
      { userId: "u1", logicalPath: "docs/a.txt", createdAt: "now" },
    ]);
    const response = await GET();
    expect(await response.json()).toEqual({
      items: [{ userId: "u1", logicalPath: "docs/a.txt", createdAt: "now" }],
    });
  ...
  it("validates origin and toggles a favorite idempotently", async () => {
    ...
    const response = await PUT(
      new Request("http://spark.test/api/discovery/favorites", {
        method: "PUT",
        headers: { origin: "http://spark.test" },
        body: JSON.stringify({ path: "docs/a.txt", favorite: true }),
      })
    );
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({
      favorite: { userId: "u1", logicalPath: "docs/a.txt", createdAt: "now" },
    });
    expect(mocks.add).toHaveBeenCalledWith({}, "u1", "docs/a.txt");
    const removeResponse = await PUT(
      new Request("http://spark.test/api/discovery/favorites", {
        method: "PUT",
        body: JSON.stringify({ path: "docs/a.txt", favorite: false }),
      })
    );
    expect(removeResponse.status).toBe(200);
    expect(await removeResponse.json()).toEqual({ favorite: null });
  ...
  it("rejects malformed logical paths", async () => {
    ...
    const response = await PUT(
      new Request("http://spark.test/api/discovery/favorites", {
        method: "PUT",
        body: JSON.stringify({ path: "../secret", favorite: true }),
      })
    );

...
export async function GET(
  request: Request = new Request("http://spark.local")
): Promise<Response> {
  const user = await (async () => {
    try {
      return (await authorizeCapability(request, "favorite")).user;
    } catch {
      return null;
    }
  })();
  ...
  return NextResponse.json(
    { items: listFavorites(getDatabase(), user.id) },
    { headers: noStore }
  );
}
...
export async function PUT(request: Request): Promise<Response> {
  ...
  if (parsed.data.favorite)
    return NextResponse.json(
      { favorite: addFavorite(database, user.id, logicalPath) },
      { headers: noStore }
    );

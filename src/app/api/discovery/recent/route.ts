...
export async function GET(request: Request): Promise<Response> {
  ...
  try { const access = await authorizeCapability(request, "favorite"); if (!access.user) throw new Error("UNAUTHENTICATED"); user = access.user; } catch { return NextResponse.json({ error: "UNAUTHENTICATED" }, { status: 401, headers: noStore }); }
  ...
  return NextResponse.json({ items: listRecentItems(getDatabase(), user.id, parsed.data.limit) }, { headers: noStore });
}

...
export async function GET(request: Request): Promise<Response> {
  let access;
  try {
    access = await authorizeCapability(request, "search");
  } catch (error) {
  ...
  try {
    ...
    const result = searchFiles(database, {
      query: parsed.data.q,
      pathPrefix: parsed.data.path,
    ...
    if (access.actorType === "anonymous")
      recordActivity(database, {
        actorType: "anonymous",
        action: "search",
        paths: parsed.data.path ? [parsed.data.path] : [],
        outcome: "success",
      });
    ...
    const code =
      ...
      ["INVALID_CURSOR", "INVALID_QUERY", "INVALID_PATH"].includes(
        error.message
      )
        ? error.message
        : "SEARCH_ERROR";
    return NextResponse.json(
      { error: code },
      { status: code === "SEARCH_ERROR" ? 500 : 400, headers: noStore }
    );
  }
}

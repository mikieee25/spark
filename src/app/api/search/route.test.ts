...
vi.mock("@/features/discovery/search-repository", () => ({
  searchFiles: mocks.search,
}));
vi.mock("@/features/discovery/discovery-repository", () => ({
  getIndexState: mocks.indexState,
}));
...
describe("search route", () => {
  it("requires authentication and private cache headers", async () => {
    ...
    const response = await GET(
      new Request("http://spark.test/api/search?q=alpha")
    );
  ...
  it("validates input and returns typed bounded results", async () => {
    ...
    mocks.search.mockReturnValue({
      items: [{ logicalPath: "docs/a.txt", name: "a.txt", kind: "file" }],
      nextCursor: null,
    });
    const response = await GET(
      new Request(
        "http://spark.test/api/search?q=alpha&path=docs&kind=file&limit=5"
      )
    );
  ...
  it("reports active and failed indexing without exposing internal errors", async () => {
    ...
    mocks.indexState.mockReturnValue({
      generation: 1,
      status: "error",
  ...
  it("returns explicit errors for malformed queries", async () => {
    mocks.user.mockResolvedValue({ id: "user-1", role: "user" });
    const response = await GET(
      new Request("http://spark.test/api/search?limit=0")
    );
  ...
  it("rejects unsafe logical paths before searching", async () => {
    mocks.user.mockResolvedValue({ id: "user-1", role: "user" });
    const response = await GET(
      new Request("http://spark.test/api/search?q=alpha&path=../secret")
    );
    expect(response.status).toBe(400);
    expect(await response.json()).toEqual({ error: "INVALID_REQUEST" });
    expect(mocks.search).not.toHaveBeenCalled();

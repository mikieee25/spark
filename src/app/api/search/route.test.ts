...
const mocks = vi.hoisted(() => ({ user: vi.fn(), database: vi.fn(), search: vi.fn() }));
...
beforeEach(() => { mocks.user.mockReset(); mocks.database.mockReset(); mocks.search.mockReset(); });
...
describe("search route", () => {
  ...
  it("validates input and returns typed bounded results", async () => {
    ...
    expect(await response.json()).toEqual({ items: [{ logicalPath: "docs/a.txt", name: "a.txt", kind: "file" }], nextCursor: null });

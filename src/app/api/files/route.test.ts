...
describe("file routes", () => {
  ...
  it("returns a bounded listing for the current user", async () => {
      ...
      .mockImplementation(
        async (
          ...
          options?.onTiming?.({
            ...
            symlinkCheck: {
              rootLstatMs: 0.5,
              segmentLstatsMs: 0.25,
              segmentCount: 2,
              slowestSegmentLstatMs: 0.2,
              slowestSegmentIndex: 1,
            },
            directoryStatMs: 2,
            cache: "miss",
            readdirMs: 3,
    ...
    const response = await GET(
      new Request("http://spark.test/api/files?path=Reports")
    );
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({
      path: "Reports",
      entries: [{ name: "note.txt", kind: "file" }],
    });
    expect(response.headers.get("cache-control")).toBe("private, no-store");
    expect(response.headers.get("server-timing")).toMatch(
      /^auth;dur=\d+(?:\.\d+)?, path;dur=1\.0, root-lstat;dur=0\.5, segment-lstats;dur=0\.3, segment-count;desc="2", slowest-segment-lstat;dur=0\.2, slowest-segment-index;desc="1", directory;dur=2\.0, readdir;dur=3\.0, metadata;dur=4\.0, sort;dur=5\.0, list;dur=\d+(?:\.\d+)?, recent;dur=\d+(?:\.\d+)?, total;dur=\d+(?:\.\d+)?$/
    );
  ...
  it("rejects a mutation without a valid origin", async () => {
    ...
    const response = await PATCH(
      new Request("http://spark.test/api/files", {
        method: "PATCH",
        body: "{}",
      })
    );
  ...
  it("returns unauthenticated delete without invoking storage", async () => {
    ...
    const response = await DELETE(
      new Request("http://spark.test/api/files?path=note.txt", {
        method: "DELETE",

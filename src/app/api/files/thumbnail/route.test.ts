...
vi.mock("@/features/discovery/thumbnail-service", () => ({
  createThumbnailService: () => ({ getThumbnail: mocks.thumbnail }),
}));
...
beforeEach(() => {
  ...
  mocks.config.mockReturnValue({ dataDirectory: "C:/spark-data" });
});
...
describe("thumbnail route", () => {
  it("requires auth and keeps unauthorized responses uncached", async () => {
    ...
    const response = await GET(
      new Request("http://spark.test/api/files/thumbnail?path=photo.png")
    );
  ...
  it("keeps private headers when auth or storage fails", async () => {
    ...
    mocks.user.mockResolvedValue({ id: "user-1", role: "user" });
    mocks.thumbnail.mockRejectedValue(

...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    const result = await createThumbnailService({
      storage: getFileStorage(),
      dataDirectory: loadConfig().dataDirectory,
    }).getThumbnail(normalizedPath);
    ...
    const status =
      code === "THUMBNAIL_TOO_LARGE"
        ...
        : code === "NOT_FOUND"
          ? 404
          : ["INVALID_PATH", "NOT_A_FILE", "UNSUPPORTED_THUMBNAIL"].includes(
                code
              )
            ? 400
            : 500;
    return NextResponse.json(
      { error: status === 500 ? "THUMBNAIL_ERROR" : code },

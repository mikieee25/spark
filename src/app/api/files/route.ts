...
function errorResponse(error: unknown): Response {
  ...
  const status =
    code === "NOT_FOUND" || code === "RECYCLE_ENTRY_NOT_ACTIVE"
      ? 404
      : code === "CONFLICT" || code === "CONFLICT_LIMIT"
        ? 409
        : code === "ADMIN_REQUIRED"
          ? 403
          : 400;
  return NextResponse.json(
    { error: /^[A-Z0-9_]+$/.test(code) ? code : "FILESYSTEM_ERROR" },
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    access = await authorizeCapability(request, "browse");
  } catch (error) {
  ...
  try {
    ...
    return NextResponse.json(
      { path: parsed.data, entries },
      { headers: { ...noStore, "Server-Timing": serverTiming } }
    );
  } catch (error) {
...
export async function PATCH(request: Request): Promise<Response> {
  ...
  try {
    return NextResponse.json(
      await getFileService().moveFile(user, parsed.data),
      { headers: noStore }
    );
  } catch (error) {
    return errorResponse(error);
...
export async function DELETE(request: Request): Promise<Response> {
  ...
  try {
    return NextResponse.json(
      await getFileService().deleteToRecycle(user, { path: parsed.data }),
      { headers: noStore }
    );
  } catch (error) {
    return errorResponse(error);

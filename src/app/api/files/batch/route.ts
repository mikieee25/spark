...
export async function POST(request: Request): Promise<Response> {
  ...
  let paths: string[];
  try {
    paths = parsed.data.paths.map(normalizeLogicalPath);
  } catch {
    return NextResponse.json(
  ...
  if (paths.some((path) => !path) || hasOverlappingPaths(paths))
    return NextResponse.json(
      { error: "INVALID_SELECTION" },
      { status: 400, headers: noStore }
    );
  ...
  try {
    access = await authorizeCapability(
      request,
      action === "download" ? "download" : "mutate"
    );
  } catch (error) {
  ...
  if (action === "download") {
    try {
      ...
      for (const path of paths)
        if (access.user) addRecentItem(database, access.user.id, path);
      recordActivity(database, {
        ...
        paths,
        outcome: "success",
        metadata: { itemType: "batch", archive: archive.filename },
      });
  ...
  for (const path of paths) {
    try {
      ...
      failed.push({ path, error: errorCode(error) });
    }
  }

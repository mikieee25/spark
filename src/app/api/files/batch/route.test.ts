...
const mocks = vi.hoisted(() => ({
  ...
  activity: vi.fn(),
  archive: vi.fn(),
  deleteToRecycle: vi.fn(),
}));

vi.mock("@/features/access/access-policy", () => ({
  authorizeCapability: mocks.authorize,
}));
vi.mock("@/features/auth/origin", () => ({
  hasValidMutationOrigin: mocks.origin,
}));
vi.mock("@/features/files/file-runtime", () => ({
  getFileStorage: mocks.storage,
  getFileService: mocks.service,
}));
vi.mock("@/features/discovery/discovery-repository", () => ({
  addRecentItem: mocks.recent,
}));
vi.mock("@/features/activity/activity-repository", () => ({
  recordActivity: mocks.activity,
}));
vi.mock("@/features/files/folder-download", () => ({
  createSelectionArchive: mocks.archive,
}));
...
describe("batch file route", () => {
  it("downloads a selected file/folder set as a private ZIP", async () => {
    mocks.archive.mockResolvedValue({
      ...
      filename: "spark-selected-files.zip",
    });

    const response = await POST(
      request({ action: "download", paths: ["Reports", "note.txt"] })
    );
  ...
  it("requires a valid origin before recycling selected items", async () => {
    ...
    const response = await POST(
      request({ action: "recycle", paths: ["note.txt"] })
    );
  ...
  it("returns per-item recycle successes and failures", async () => {
    mocks.deleteToRecycle.mockImplementation(
      async (_user: unknown, { path }: { path: string }) => {
    ...
    const response = await POST(
      request({ action: "recycle", paths: ["good.txt", "bad.txt"] })
    );

    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({
      succeeded: ["good.txt"],
      failed: [{ path: "bad.txt", error: "NOT_FOUND" }],
      undo: [{ id: "recycle-good.txt", path: "good.txt" }],
    });
  });

  it("rejects duplicate paths", async () => {
    const response = await POST(
      request({ action: "download", paths: ["note.txt", "note.txt"] })
    );
  ...
  it("rejects overlapping paths even when another sibling sorts between them", async () => {
    const response = await POST(
      request({ action: "download", paths: ["A", "A-b.txt", "A/file.txt"] })
    );

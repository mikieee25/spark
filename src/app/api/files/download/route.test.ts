...
vi.mock("@/features/files/folder-download", () => ({
  createFolderArchive: mocks.archive,
}));
...
describe("download route", () => {
  it("records a recent item only after a successful authenticated download", async () => {
    ...
    const response = await GET(
      new Request("http://spark.test/api/files/download?path=docs/a.txt")
    );
  ...
  it("does not record failed downloads", async () => {
    mocks.user.mockResolvedValue({ id: "u1", role: "user" });
    mocks.storage.mockReturnValue({
      stat: vi.fn().mockRejectedValue(new Error("NOT_FOUND")),
    });
    const response = await GET(
      new Request("http://spark.test/api/files/download?path=docs/a.txt")
    );
  ...
  it("does not record a recent item when reading the file fails", async () => {
    ...
    mocks.storage.mockReturnValue({
      stat: vi.fn().mockResolvedValue({ kind: "file" }),
      readFile: vi.fn().mockRejectedValue(new Error("READ_FAILED")),
    });
    const response = await GET(
      new Request("http://spark.test/api/files/download?path=docs/a.txt")
    );
  ...
  it("downloads a folder as a ZIP and records the recent item", async () => {
    mocks.user.mockResolvedValue({ id: "u1", role: "user" });
    mocks.storage.mockReturnValue({
      stat: vi.fn().mockResolvedValue({ kind: "folder" }),
    });
    ...
    const response = await GET(
      new Request("http://spark.test/api/files/download?path=Reports")
    );
    expect(response.status).toBe(200);

...
const clientErrors = new Set([
  ...
  "NOT_A_FILE",
  "UNSUPPORTED_PREVIEW",
  "PREVIEW_CONVERTER_DISABLED",
  "PREVIEW_CONVERTER_UNAVAILABLE",
  "PREVIEW_CONVERTER_INVALID_RESPONSE",
  "PREVIEW_TOO_LARGE",
]);
...
function errorResponse(error: unknown): Response {
  ...
  const status =
    raw === "NOT_FOUND"
      ? 404
      : raw === "PREVIEW_TOO_LARGE"
        ? 413
        : clientErrors.has(raw)
          ? 400
          : 500;
  return NextResponse.json(
    { error: clientErrors.has(raw) || status !== 500 ? raw : "PREVIEW_ERROR" },
...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    const service = createPreviewService(getFileStorage(), {
      converter: config.previewConverterUrl
        ? createDocumentConverter(config.previewConverterUrl)
        : undefined,
    ...
    headers.set(
      "Content-Length",
      String(result.body?.byteLength ?? result.totalBytes)
    );
    headers.set(
      "Content-Disposition",
      `inline; filename*=UTF-8''${encodeURIComponent(`${path.posix.basename(logicalPath)}.pdf`)}`
    );
    return new Response(
      result.body ? (new Uint8Array(result.body) as unknown as BodyInit) : null,
      { status: 200, headers }
    );
  } catch (error) {

...
function errorResponse(error: unknown): Response {
  ...
  const status = code === "PREVIEW_TOO_LARGE" || code === "ARCHIVE_TOO_LARGE" ? 413 : code === "RANGE_NOT_SATISFIABLE" ? 416 : code === "NOT_FOUND" ? 404 : clientErrors.has(code) ? 400 : 500;
  return NextResponse.json({ error: clientErrors.has(code) || status !== 500 ? code : "PREVIEW_ERROR" }, { status, headers: noStore });
}
...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    const result = await createPreviewService(getFileStorage()).preview(normalizedPath, request.headers.get("range") ?? undefined);
    if (access.user) addRecentItem(getDatabase(), access.user.id, normalizedPath);
    else recordActivity(getDatabase(), { actorType: "anonymous", action: "file_preview", paths: [normalizedPath], outcome: "success" });
    if (result.kind !== "media") return NextResponse.json(result, { headers: noStore });
    const headers = new Headers(noStore);
    headers.set("Content-Type", result.mimeType);
    headers.set("Content-Length", String(result.partial ? result.body?.byteLength ?? 0 : result.totalBytes));
    headers.set("Accept-Ranges", "bytes");
    headers.set("Content-Disposition", `inline; filename*=UTF-8''${encodeURIComponent(path.posix.basename(result.logicalPath))}`);
    if (result.partial) headers.set("Content-Range", `bytes ${result.start}-${result.end}/${result.totalBytes}`);
    const body = result.body ?? (result.stream ? Readable.toWeb(result.stream) : null);
    if (!body) throw new Error("PREVIEW_ERROR");
    return new Response(body as unknown as BodyInit, { status: result.partial ? 206 : 200, headers });
  } catch (error) {

...
vi.mock("@/features/discovery/preview-service", () => ({
  createPreviewService: () => ({ preview: mocks.preview }),
}));
...
describe("preview route", () => {
  it("requires auth and private no-store responses", async () => {
    ...
    const response = await GET(
      new Request("http://spark.test/api/files/preview?path=note.txt")
    );
  ...
  it("returns range headers for inline media", async () => {
    ...
    const response = await GET(
      new Request("http://spark.test/api/files/preview?path=clip.mp4", {
  ...
  it("returns an explicit unsupported response for SVG previews", async () => {
    ...
    const response = await GET(
      new Request("http://spark.test/api/files/preview?path=unsafe.svg")
    );

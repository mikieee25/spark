...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }): Promise<Response> {
  ...
  const parsed = z.object({ conflict: z.enum(["fail", "replace", "rename", "skip"]).optional() }).safeParse(body);
  if (!parsed.success) return NextResponse.json({ error: "INVALID_REQUEST" }, { status: 400, headers: noStore });
  try { return NextResponse.json(await getFileService().restoreFromRecycle(user, { id: (await context.params).id, conflict: parsed.data.conflict }), { headers: noStore }); }

...
describe("recycle routes", () => {
  ...
  it("does not allow non-admin purge", async () => {
    ...
    const response = await DELETE(new Request("http://spark.test/api/recycle/entry", { method: "DELETE", headers: { origin: "http://localhost:3000" } }), { params: Promise.resolve({ id: "entry" }) });

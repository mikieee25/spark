...
export async function GET(): Promise<Response> {
  ...
  const report = checkAdminHealth({
    database: () => Boolean(getDatabase().prepare("SELECT 1").get()),
    filesRoot: () => {
    ...
    settings: () => {
      getRetentionDays(getDatabase());
      return true;
    },
    indexState: getIndexState(getDatabase()),
  });
  return NextResponse.json(
    { ...report, runtimePerformance: getRuntimePerformanceSnapshot() },
    { headers }
  );
}

...
const userSchema = z.object({ username: z.string().trim().min(1).max(128), displayName: z.string().trim().min(1).max(160), password: z.string().min(12).max(128), role: z.enum(["user", "admin"]).optional() });
function errorResponse(error: unknown): Response { const code = error instanceof Error ? error.message : "ADMIN_OPERATION_FAILED"; const status = code === "ADMIN_REQUIRED" ? 403 : code === "USER_NOT_FOUND" ? 404 : code === "USERNAME_EXISTS" ? 409 : code === "LAST_ADMIN" ? 409 : 400; return NextResponse.json({ error: /^[A-Z0-9_]+$/.test(code) ? code : "ADMIN_OPERATION_FAILED" }, { status, headers }); }
async function admin() { const user = await getCurrentUser(); return user?.role === "admin" ? user : null; }

export async function GET(): Promise<Response> { const user = await admin(); if (!user) return NextResponse.json({ error: "ADMIN_REQUIRED" }, { status: 403, headers }); return NextResponse.json({ users: listUsers(getDatabase(), user) }, { headers }); }
export async function POST(request: Request): Promise<Response> {
  ...
  const parsed = userSchema.safeParse(body); if (!parsed.success) return NextResponse.json({ error: "INVALID_REQUEST" }, { status: 400, headers });
  try { return NextResponse.json({ user: await createUser(getDatabase(), user, parsed.data) }, { status: 201, headers }); } catch (error) { return errorResponse(error); }
}

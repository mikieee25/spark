...
async function main(): Promise<void> {
  ...
  if (!username)
    throw new Error("Usage: npm run admin:reset-password -- --username <name>");
  ...
  if (user.role !== "admin")
    throw new Error(`Account is not an administrator: ${user.username}`);
  if (user.disabled_at)
    throw new Error(`Administrator account is disabled: ${user.username}`);
  ...
  process.stdout.write(
    `Temporary password set for ${user.username}. Sign in with it once, then choose a new password.\n`
  );
}

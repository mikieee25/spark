...
test("searches, previews, favorites, and records a recent item", async ({
  page,
}) => {
  await signIn(page);
  ...
  await expect(
    page.getByRole("button", { name: /Open favorite search-fixture\.txt/i })
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: /Open recent item search-fixture\.txt/i })
  ).toBeVisible();
});
...
test("supports mobile search controls", async ({ page }) => {
  ...
  await signIn(page);
  await expect(
    page.getByRole("searchbox", { name: "Search workspace" })
  ).toBeVisible();
  await expect(
